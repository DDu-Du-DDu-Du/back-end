...
class UpdateReminderServiceTest {
  ...
  void 미리알림_갱신에_성공한다() {
    // given
    LocalDateTime newRemindsAt = todo.getScheduledOn()
    ...
    // then
    Reminder updated = reminderLoaderPort.getOptionalReminder(reminder.getId())
        .orElseThrow();
  ...
  void 투두가_없으면_실패한다() {
    ...
    UpdateReminderRequest request = new UpdateReminderRequest(LocalDateTime.now()
        .plusDays(1));
