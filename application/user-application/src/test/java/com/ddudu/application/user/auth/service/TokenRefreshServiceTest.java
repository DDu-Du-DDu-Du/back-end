...
class TokenRefreshServiceTest {
  ...
  void 토큰_갱신을_성공한다() {
    ...
    Assertions.assertThat(refreshTokens).anyMatch(token -> token.hasSameTokenValue(actual.refreshToken()));
  }
  ...
  void 저장된_토큰이_없으면_토큰_갱신을_실패한다() {
    ...
    // then
    Assertions.assertThatExceptionOfType(UnsupportedOperationException.class).isThrownBy(refresh)
  ...
  void 토큰의_사용자를_찾을_수_없으면_토큰_갱신을_실패한다() {
    ...
    // then
    Assertions.assertThatExceptionOfType(MissingResourceException.class).isThrownBy(refresh)
