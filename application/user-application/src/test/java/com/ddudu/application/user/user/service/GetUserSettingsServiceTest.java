...
class GetUserSettingsServiceTest {
  ...
  void 내_세팅_불러오기를_성공한다() {
    ...
    // then
    assertThat(actual.display().weekStartDay()).isEqualTo(expected.getWeekStartDay());
    assertThat(actual.display().isDarkMode()).isEqualTo(expected.isDarkMode());
    assertThat(actual.menuActivation().calendar().isActive())
        .isEqualTo(expected.isActiveCalendar());
    assertThat(actual.menuActivation().calendar().priority())
        .isEqualTo(expected.getPriorityCalendar());
    assertThat(actual.menuActivation().dashboard().isActive())
        .isEqualTo(expected.isActiveDashboard());
    assertThat(actual.menuActivation().dashboard().priority())
        .isEqualTo(expected.getPriorityDashboard());
    assertThat(actual.menuActivation().stats().isActive())
        .isEqualTo(expected.isActiveStats());
    assertThat(actual.menuActivation().stats().priority())
        .isEqualTo(expected.getPriorityStats());
    assertThat(actual.appConnection().realtimeSync().notion())
        .isEqualTo(expected.isRealtimeSyncNotion());
    assertThat(actual.appConnection().realtimeSync().googleCalendar())
        .isEqualTo(expected.isRealtimeSyncGoogleCalendar());
    assertThat(actual.appConnection().realtimeSync().microsoftTodo())
