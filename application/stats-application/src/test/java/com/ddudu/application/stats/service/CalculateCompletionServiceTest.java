...
class CalculateCompletionServiceTest {
  ...
  void 특정_날짜_사이의_자신의_할_일_달성률_조회를_성공한다() {
    ...
    // then
    Assertions.assertThat(responses).hasSize(7);
  ...
  void 로그인_아이디가_존재하지_않아_주간_할_일_달성률_조회를_실패한다() {
    ...
    // then
    AssertionsForClassTypes.assertThatExceptionOfType(MissingResourceException.class).isThrownBy(findWeeklyCompletions)
