...
class CollectMonthlyStatsSummaryServiceTest {
  ...
  class 만든_수_통계_테스트 {
    ...
    void 이번_달_월별_목표들의_투두_생성_수_통계를_내림차순으로_낸다() {
      ...
      // then
      List<Integer> actualCreationCounts = response.summaries()
          .stream()
  ...
  class 지속도_통계_테스트 {
    ...
    void 이번_달_지속한_투두_수_통계를_내림차순으로_반환한다() {
      ...
      // then
      List<Integer> actualSustenanceCounts = response.summaries()
          .stream()
  ...
  class 미루기_통계_테스트 {
    ...
    void 월_목표_별_미루기_통계를_반환한다() {
      ...
      // then
      List<Integer> actualPostponedCounts = response.summaries()
          .stream()
    ...
    void 미루기_투두가_없는_달이면_통계는_비어있다() {
      // given
      YearMonth noTodoMonth = YearMonth.now()
          .minusMonths(2);
  ...
  class 재달성_통계_테스트 {
    ...
    void 월_목표_별_재달성_수_통계를_반환한다() {
      ...
      // then
      List<Integer> actualReattainmentCounts = response.summaries()
          .stream()
  ...
  void 요청_사용자_아이디가_있으면_로그인_사용자_대신_해당_사용자_기준으로_월_요약을_조회한다() {
    ...
    saveTodoPort.saveAll(List.of(
        TodoFixture.createRandomTodoWithReference(
            targetGoal.getId(), targetUser.getId(), true,
            TodoStatus.COMPLETE
        ),
        TodoFixture.createRandomTodoWithReference(
            targetGoal.getId(), targetUser.getId(), false,
    ...
    assertThat(response.summaries()
        .get(0)
        .goalColor()).isEqualTo(targetGoal.getColor());
  }
