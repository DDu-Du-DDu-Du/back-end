...
public class TodoFixture extends BaseFixture {
  ...
  public static List<Todo> createTodosWithPostponedFlag(
    ...
    for (int i = 0; i < postponedCount; i++) {
      ...
      todos.add(createTodoWithScheduleAndPostponedFlag(goal, true, scheduledOn));
    }
    ...
    for (int i = 0; i < notPostponedCount; i++) {
      ...
      todos.add(createTodoWithScheduleAndPostponedFlag(goal, false, scheduledOn));
    }
  ...
  public static List<Todo> createConsecutiveCompletedTodos(Goal goal, int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      todos.add(createRandomTodoWithStatusAndSchedule(goal, TodoStatus.COMPLETE, scheduled));
    }
  ...
  public static List<Todo> createDifferentTodosWithGoal(
    ...
    todos.addAll(createMultipleTodosWithGoal(goal, completedCount));
    todos.addAll(createMultipleTodosWithGoal(goal, uncompletedCount, TodoStatus.UNCOMPLETED));
  ...
  private static List<Todo> createMultipleTodosWithGoal(Goal goal, int size, TodoStatus status) {
    ...
    for (int i = 0; i < size; i++) {
      todos.add(createRandomTodoWithStatus(goal, status));
    }
