...
class OptionSubVoTest {
  ...
  void DisplayOptions의_기본값을_생성한다() {
    ...
    // when
    DisplayOptions actual = DisplayOptions.builder()
        .build();
  ...
  void MenuActivationOptions의_기본값을_생성한다() {
    ...
    // when
    MenuActivationOptions actual = MenuActivationOptions.builder()
        .build();

    // then
    assertThat(actual.getCalendar()
        .isActive()).isTrue();
    assertThat(actual.getCalendar()
        .getPriority()).isEqualTo(1);
    assertThat(actual.getDashboard()
        .isActive()).isTrue();
    assertThat(actual.getDashboard()
        .getPriority()).isEqualTo(2);
    assertThat(actual.getStats()
        .isActive()).isTrue();
    assertThat(actual.getStats()
        .getPriority()).isEqualTo(3);
  }
  ...
  void AppConnectionOptions의_기본값을_생성한다() {
    ...
    // when
    AppConnectionOptions actual = AppConnectionOptions.builder()
        .build();

    // then
    assertThat(actual.getRealtimeSync()
        .isNotion()).isFalse();
    assertThat(actual.getRealtimeSync()
        .isGoogleCalendar()).isFalse();
    assertThat(actual.getRealtimeSync()
        .isMicrosoftTodo()).isFalse();
  }
